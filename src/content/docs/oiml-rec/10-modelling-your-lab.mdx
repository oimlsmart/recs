---
title: 'Modelling Your Lab'
shortTitle: 'Modelling Your Lab'
description: 'The laboratory’s SOPs, equipment register and record forms as an implementation package (a fictional worked-example laboratory).'
eyebrow: 'Authoring · 10 of 17'
order: 10
---
...
> The worked example is **MTL Rhein Ruhr**, a fictional
> Scheme-A load-cell laboratory mapped to R 60-2, invented for this
> walkthrough and existing nowhere else.
...
organization:
  ...
  scheme: A
  accreditation: "EXAMPLE D-PL-19283-01-00 — ISO/IEC 17025, … (fictional)"
maps_to:
  standard: r60
  methods:
...
equipment:
  - id: mtl_f_001
    ...
    calibration:
      certificate: "Example TL calibration certificate KA-2025-1187"
      calibrated_on: "2025-11-03"
      due_on: "2026-11-02"
      traceable_to: "the national force standard"
```
...
- `coerce: int-bool`, the lab records a **count** (integer) where the
  ...
  coerces to true. The example's `contact_discharge_count` and
...
- `mapping:`, a **multi-valued or free-text** recording reducing to a
  **single-valued** required field (an enum, or any scalar): the note
  states how. The example's `polarity_applied` records 'positive' /
...
The example's report, computed by the calculus:
...
1. walk the steps in order at the example run date (2026-06-08): every
...
judgments documents none of them. The example therefore maps each method
...
*Next: [Volume IV, the OIML-CS Scheme](/docs/oiml-cs/) and the
[platform annex](https://www.primmel.org/primmel-smart-docs/platform/), on the Primmel SMART docs site, the OIML-CS
