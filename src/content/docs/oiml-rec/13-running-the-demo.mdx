...
shortTitle: 'Running the Demo'
description: 'The demo manual: bootstrap, walkthrough, verification, run the full certification chain yourself.'
eyebrow: 'Authoring · 13 of 17'
order: 13
---
...
standalone sim instruments, and the asserted live-twin suite (the
`pilot` e2e).
...
>
> **The cast.** The demo's parties are one fictional set, defined once
> in the smart repo's `docs/demo-personas.md`: the ACME manufacturer
> family (Dr. Anna Meyer signs the applications), the Example Test
> Laboratory (Ms. Petra Horvat's team runs the tests), the Example
> Issuing Authority (Ms. Emily Irving reviews and signs), and the BIML
> side by role name only (the Registration Officer, the Executive
> Secretary). No real organization or person appears anywhere in a demo
> flow; the OIML-CS registry and the certificate corpus stay real
> reference data.
...
Seeded 7 demo accounts
  ...
  applicant@oiml.org — ACME Applicant (applicant, org: mfr-acme)
  ia@oiml.org — IA Officer (ia_officer, org: EX1)
  tl@oiml.org — TL Operator (tl_operator, org: 21)
...
| `applicant@oiml.org` | applicant (ACME) | the applicant portal, apply for certification |
| `ia@oiml.org` | IA officer | the Issuing Authority console, dispatch, evaluate, sign |
| `tl@oiml.org` | TL operator | the lab workbench, test requests, test runs, forms |
...
1. **Applicant portal** (as `applicant@oiml.org`): the applications
   ...
   covers.
2. **Lab inbox / workbench** (as `tl@oiml.org`): the test laboratory's test requests
