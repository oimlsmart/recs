---
title: 'Authoring OIML Recommendations'
shortTitle: 'Overview'
description: 'From Recommendation text to a validated package: the methodology, the gates, the walkthroughs.'
eyebrow: 'Authoring · Volume overview'
order: 0
---
...
running system: R 60 is the worked example throughout; R 91 (traffic speed meters)
and R 144 (gas analytical systems) are the stress cases.
...
Where the content is not the rec's own normative text, you do not author it: core
...
1. [The authoring method](/docs/oiml-rec/01-methodology), the end-to-end method in
   seven moves; the validation gates; the pitfalls catalog.
2. [Modelling the subject](/docs/oiml-rec/02-modelling-the-subject), subject types,
   variants, dimensions, attributes (origin/scope/category), capabilities,
   behaviors, conditions.
3. [Requirements](/docs/oiml-rec/03-requirements), anatomy; binding; OCL limits;
   applicability; verification methods; tables and profiles.
4. [Conformance tests](/docs/oiml-rec/04-conformance-tests), variables and sources;
   steps; conditions; acceptance criteria; inheritance; instances; kinds
   and obligation.
5. [Forms and reports](/docs/oiml-rec/05-forms-and-reports), bind paths; measurement
   methods; pass/fail; the test-report skeleton; checklists; omissions.
6. [Evaluation](/docs/oiml-rec/06-evaluation), determinations, verdicts, model
   evaluation, decision; the certificate template; approvals.
7. [Packaging](/docs/oiml-rec/07-packaging), the package layout; `uses` composition;
   sample data; registration.
8. [Walkthrough: R 60](/docs/oiml-rec/08-walkthrough-r60), the worked example, end to
   end.
9. [Walkthrough: R 91 and R 144](/docs/oiml-rec/09-walkthrough-r91-r144), traffic
   speed meters and gas analytical systems: the stress cases.
10. [Modelling your lab](/docs/oiml-rec/10-modelling-your-lab), the laboratory's
    SOPs, equipment register and record forms as an implementation
    package; the `.prm` to the required methods; full cover by the
    calculus (a fictional worked-example laboratory).
11. [Migrating from the YAML era](/docs/oiml-rec/11-migrating-from-yaml), the two
...
12. [Modelling your product](/docs/oiml-rec/12-modelling-your-product), the
    ...
    twin (the ACME LC-500 worked example; spec: Volume I, chapter 15).
